...
def upgrade() -> None:
    ...
    for schema in schemas:
        # Skip if batch_jobs doesn't exist (new tenant or already migrated)
        if not table_exists(conn, schema, 'batch_jobs'):
            continue

        # Skip if marketplace_batches already exists (already migrated)
        if table_exists(conn, schema, 'marketplace_batches'):
            continue

        # 2. Rename the batch_jobs table to marketplace_batches
