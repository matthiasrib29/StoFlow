...
export const useEbayPolicies = () => {
  ...
  const fetchPolicies = async (): Promise<{
    ...
    const [shipping, returns, payment] = await Promise.all([
      api.get<EbayShippingPolicy[]>('/api/integrations/ebay/policies/shipping'),
      api.get<EbayReturnPolicy[]>('/api/integrations/ebay/policies/return'),
      api.get<EbayPaymentPolicy[]>('/api/integrations/ebay/policies/payment')
    ])
