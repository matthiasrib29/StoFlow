...
2. **Files iterating through schemas manually** (this plan)
   ...
   - Need direct `SET search_path` → `execution_options(schema_translate_map=...)`
...
- [x] **Task 1**: Review and modify schedulers ✅
  - `job_cleanup_scheduler.py` - Added execution_options for ORM queries
  - `datadome_scheduler.py` - Added execution_options for ORM queries
  - `etsy_polling_cron.py` - Replaced set_user_schema with execution_options

- [x] **Task 2**: Modify `api/products/ai.py` ✅
  - Removed redundant SET call (schema already configured via get_user_db)

- [x] **Task 3**: Analyze and modify `api/ebay/products.py` ✅
  - Analyzed: Background tasks create own sessions
  - Added execution_options at start for ORM queries
  - Kept ONE SET at start for text() queries
  - Removed all redundant re-SET calls throughout

- [x] **Task 4**: Modify `services/ebay/jobs/ebay_orders_sync_job_handler.py` ✅
  - Removed redundant `set_user_schema` call (job processor already configures schema)

- [x] **Task 5**: Modify scripts ✅
  - `cleanup_abandoned_drafts.py` - Added execution_options, removed reset to public
  - `migrate_existing_jobs_to_batch.py` - Added execution_options
  - `migrate_oauth_tokens_encryption.py` - Replaced SET with execution_options
  - `migrate_from_pythonapiwoo.py` - Added execution_options

- [x] **Task 6**: Verify changes ✅
  ```bash
  grep -r "set_user_schema\|set_user_search_path\|from shared.schema_utils import" backend/ --include="*.py"
  # No deprecated patterns found in application code
  ```
